...
- **Constraint:** You must use the exact CLI flags and subcommands defined by the `castra` protocol. Do not guess or hallucinate command structures. DO NOT USE the `./castra` workspace binary but always use the `castra` system command.
